...
with open(sys.argv[1]) as f:
    with open('inMAG_author_pairs.tsv', 'w') as fo:
        for line in f:
            ...
            if len(in_lang) > 0 and len(d_mid) > 0 and len(g_mid) > 0:
                    ...
                    if citing_authors == None:
                        ...
                        g_auth_list = ' | '.join(['{} ({})'.format(ca[1], ca[0]) for ca in citing_authors])
                    if cited_authors == None:
                        d_auth_list = '<None>'
                    else:
                        d_auth_list = ' | '.join(['{} ({})'.format(ca[1], ca[0]) for ca in cited_authors])
                    fo.write(f'{author_overlap}\t{g_auth_list}\t{d_auth_list}\t{d_mid}\n')
